...
const listReadyHitTaskBpjsPoliSesuai = async (limit: number, task_id: number) => {
    let filter, selectTaskTime, kondisiTask, filterCheckin;
    ...
    if (task_id === 1) {
        ...
        kondisiTask = ` inner join rujukan_sep on 
                        ...
                        inner join emr on 
                            ...
                            and emr.form_id = 6 
                            and emr.status_batal is null`;
        filterCheckin = `and tgl_check_in is not null`;
    } else if (task_id === 4) {
        filter = `and last_task = '3'`;
        selectTaskTime = `,(emr.input_time) task_time`;
        kondisiTask = ` inner join emr on 
    ...
    const queryTask = `WITH data_task as (
        select
            ...
            max(peresepan_obat.status_selesai) status_obat,
            registrasi.tgl_masuk::date,
            rujukan_pasien.kode_poli_bpjs kode_poli_rujukan,
            mapping_poli_bpjs.kode_poli_bpjs kode_poli_mapping,
            registrasi_urut.tgl_check_in
        from
            registrasi
        inner join pasien on
            registrasi.pasien_id = pasien.pasien_id
        ...
        left join rujukan_sep on
            registrasi.registrasi_id = rujukan_Sep.registrasi_id
        left join rujukan_pasien on
            rujukan_sep.no_rujukan = rujukan_pasien.no_rujukan 
            and rujukan_pasien.status_batal is null
        left join mapping_poli_bpjs on
        ...
        from
            ...
            where
                1 = 1
                and kode_poli_rujukan = kode_poli_mapping
                ${filter}
                ${filterCheckin}
            ) as data_log
        where 
...
const listReadyHitTaskBpjsRujukBedaPoli = async (limit: number, task_id: number) => {
    let filter, selectTaskTime, kondisiTask;
