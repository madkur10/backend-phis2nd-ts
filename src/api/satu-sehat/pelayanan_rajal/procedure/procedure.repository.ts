...
const getProcedureRadRequest = async (limit: string, hasil_rad_detail_id: string = "") => {
    let queryRegistrasiId;
    let queryDate;
    let queryWhereTransaction;
    if (hasil_rad_detail_id) {
        queryDate = "";
        queryRegistrasiId = `AND tss_procedure_rad.hasil_rad_detail_id = ${parseInt(
            hasil_rad_detail_id,
            10
        )}`;
    ...
    const getDataPasien = `
        SELECT
            ...
            tss_procedure_rad.transaction_satu_sehat_id
        from
            order_rad
        ...
        left join transaction_satu_sehat tss_procedure_rad on
            hasil_rad_detail.hasil_rad_detail_id = tss_procedure_rad.key_simrs 
            and tss_procedure_rad.transaction_type = 'ProcedureRad'
        where
...
const updateInsertIdProcedureRadRepo = async (
...
const updateUpdateIdProcedureRadRepo = async (
    hasil_rad_detail_id: bigint,
    payload: any,
    response: any,
    id: string,
